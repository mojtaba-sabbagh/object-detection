/**
 * Minimal MD5 (RFC 1321) returning a lowercase hex digest.
 *
 * Needed because VoTT derives an asset's id from an MD5 of its file URI, and
 * the Web Crypto API deliberately omits MD5. Input is hashed as UTF-8, which
 * matches the `md5` npm package VoTT uses.
 */
...
function rotateLeft(num: number, cnt: number): number {
...
/** Hashes an array of little-endian 32-bit words of a message `len` bits long. */
...
function bytesToWords(bytes: Uint8Array): number[] {
  const words: number[] = new Array(((bytes.length + 8) >> 6) * 16 + 16).fill(0);
...
function wordsToHex(words: number[]): string {
  ...
  for (const word of words) {
    for (let byte = 0; byte < 4; byte++) {
      ...
      out += hexDigits.charAt(value >>> 4) + hexDigits.charAt(value & 0x0f);
    }
  }
  return out;
}
